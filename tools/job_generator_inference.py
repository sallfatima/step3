...
def generate_prediction_job_argument(ward_id: str, coordinates: List[List[float]], 
    ...
    """
    Génère un argument de job de prediction pour un ward donné
    ...
    Returns:
        Argument de job formaté pour inference/prediction
    """
    ...
    # Formater l'argument de job pour inference/prediction
    job_parts = [
...
def generate_all_prediction_jobs(ward_coordinates: Dict[str, List[List[float]]], 
                               output_file: str = "johannesburg/inference_predict.json",
                               area_prefix: str = "africa/south_africa/johannesburg",
                               custom_suffix: str = "johannesburg_custom_ward",
                               annotations_filename: str = None,
                               custom_polygon_filename: str = None) -> None:
    """
    Génère tous les arguments de job de prediction pour les wards
    """
    print(f"🤖 GÉNÉRATION DES ARGUMENTS DE JOB INFERENCE/PREDICTION")
    print("=" * 60)
    ...
    for ward_id, coordinates in sorted_wards:
        job_arg = generate_prediction_job_argument(
            ward_id, coordinates, area_prefix, custom_suffix, annotations_filename, custom_polygon_filename
        )
        ...
        print(f"✅ Ward {ward_number} ({ward_id}): Job prediction généré")
...
"""
    ...
    print(f"   📊 Arguments: {len(job_arguments)} jobs de prediction")
    print(f"   🚀 Pipeline: INFERENCE/PREDICTION")

def generate_prediction_zone_configs(ward_coordinates: Dict[str, List[List[float]]]) -> None:
    """
    Génère des configurations de prediction séparées par zones géographiques
    Crée des blocs de 50 wards maximum par zone pour les predictions
    """
    print(f"\n🎯 GÉNÉRATION DE CONFIGURATIONS PAR ZONES")
    print("=" * 60)
    
    # Définir les zones par blocs de 50 wards pour la prediction
    zones = {
        "zone_001_050": {
        ...
        "zone_051_100": {
            "wards": list(range(51, 101)),
            "annotations": "annotations_zone_2.json"
        },
        "zone_101_150": {
            "wards": list(range(101, 151)),
            "annotations": "annotations_zone_3.json"
        },
        "zone_151_200": {
            "wards": list(range(151, 201)),
            "annotations": "annotations_zone_4.json"
        },
        "zone_201_250": {
            "wards": list(range(201, 251)),
            "annotations": "annotations_zone_5.json"
        },
        "zone_251_300": {
            "wards": list(range(251, 301)),
            "annotations": "annotations_zone_6.json"
        },
        "zone_301_350": {
            "wards": list(range(301, 351)),
            "annotations": "annotations_zone_7.json"
        }
    }
    ...
    for zone_name, zone_config in zones.items():
        ...
        if zone_wards:
            zone_file = f"johannesburg/inference_predict_{zone_name}.json"
            generate_all_prediction_jobs(
                zone_wards, 
                zone_file,
...
def print_sample_prediction_jobs(ward_coordinates: Dict[str, List[List[float]]], sample_count: int = 3) -> None:
    """
    Affiche quelques exemples d'arguments de job de prediction
    """
    print(f"\n📋 EXEMPLES D'ARGUMENTS DE JOB PREDICTION GÉNÉRÉS:")
    print("=" * 60)
    
    sample_wards = list(ward_coordinates.items())[:sample_count]
    
    for ward_id, coordinates in sample_wards:
        job_arg = generate_prediction_job_argument(ward_id, coordinates)
        print(job_arg)
    
    if len(ward_coordinates) > sample_count:
        print(f"   ... et {len(ward_coordinates) - sample_count} autres wards")

def generate_single_prediction_job(ward_id: str, coordinates: List[List[float]], 
                                 annotations_filename: str = None) -> str:
    """
    Génère un argument de job de prediction pour un seul ward (utile pour tests)
    ...
    """
    job_arg = generate_prediction_job_argument(ward_id, coordinates, annotations_filename=annotations_filename)
    
    print(f"🎯 ARGUMENT DE JOB PREDICTION POUR LE WARD {ward_id}:")
    print("=" * 50)
    print(job_arg)
    
    return job_arg

def generate_custom_prediction_configs(ward_coordinates: Dict[str, List[List[float]]]) -> None:
    """
    Génère différentes configurations de prediction personnalisées
    """
    print(f"\n🎨 GÉNÉRATION DE CONFIGURATIONS PERSONNALISÉES")
    print("=" * 60)
    
    # Configuration 1: Prediction avec annotations globales
    print("\n1️⃣ Configuration avec annotations globales...")
    generate_all_prediction_jobs(
        ward_coordinates, 
        "johannesburg/inference_predict_global.json",
        annotations_filename="annotations_johannesburg_global.json"
    )
    
    # Configuration 2: Prediction par zones
    print("\n2️⃣ Configuration par zones...")
    generate_prediction_zone_configs(ward_coordinates)
    
    # Configuration 3: Prediction avec annotations par date
    print("\n3️⃣ Configuration avec annotations par date...")
    from datetime import datetime
    current_date = datetime.now().strftime("%Y%m%d")
    generate_all_prediction_jobs(
        ward_coordinates,
        f"johannesburg/inference_predict_{current_date}.json",
        annotations_filename=f"annotations_johannesburg_{current_date}.json"
    )

def main():
    """
    Fonction principale pour la génération des jobs de prediction
    """
    print("🤖 GÉNÉRATEUR D'ARGUMENTS DE JOB INFERENCE/PREDICTION - JOHANNESBURG")
    print("=" * 70)
    ...
    print_sample_prediction_jobs(ward_coordinates)
    
    # Générer la configuration principale pour PREDICTION
    print(f"\n🚀 GÉNÉRATION DE LA CONFIGURATION PRINCIPALE")
    print("=" * 60)
    generate_all_prediction_jobs(ward_coordinates)
    
    # Générer les configurations personnalisées
    generate_custom_prediction_configs(ward_coordinates)
    
    print(f"\n🎉 GÉNÉRATION TERMINÉE!")
    print(f"✅ {len(ward_coordinates)} arguments de job de prediction générés")
    ...
    print(f"   4. Fichier principal: johannesburg/inference_predict.json")
    print(f"   5. Chaque ward aura son fichier: annotations_ward_n.json")
    print(f"   6. Ou: for ward_id, args in cli_args_per_job.items() pour tous")
    
    # Exemple d'un seul ward
    if ward_coordinates:
        first_ward_id = list(ward_coordinates.keys())[0]
        first_ward_coords = ward_coordinates[first_ward_id]
        print(f"\n🔍 EXEMPLE D'ARGUMENT DE JOB PREDICTION:")
        generate_single_prediction_job(first_ward_id, first_ward_coords)
