...
resource "google_pubsub_topic" "batch_job_notifications" {
  name = "batch-job-status-updated-inference-pipeline-joh"
}
...
#   name        = "geomapping-inference-send-failure-email"
...
resource "google_cloudbuild_trigger" "batch_job_destroy_trigger" {
  ...
  name     = "trigger-inference-destroy-${each.key}"
  location = var.location
  ...
  build {
    logs_bucket = "gs://lengo-geomapping/cloud_build_logs/inference"
    ...
    step {
      ...
      args       = [
        ...
        # "for job_name in $(gcloud batch jobs list --filter='labels.area_name=\"${each.key}\" AND labels.source=\"inference_pipeline\"' --sort-by=~createTime --format='value(name)'); do gcloud batch jobs delete \"$job_name\" --location=\"${var.location}\" --quiet || echo \"Failed to delete job $job_name\"; done"
      ]
    }
  }
}
...
resource "google_cloud_scheduler_job" "batch-job-invoker" {
  ...
  name             = "geomapping-inference-scheduler-invoker-${each.key}"
  description      = "Run inference pipeline job for ${each.key}"
  project          = var.PROJECT_ID
  region           = var.location
  schedule         = local.cron
      ...
      "taskGroups":[
        {
          "taskSpec": {
            "runnables":[
              {
                "container": {
                  "imageUri": "${var.location}-docker.pkg.dev/${var.PROJECT_ID}/geomapping/inference_pipeline:${data.external.git.result.sha}",
