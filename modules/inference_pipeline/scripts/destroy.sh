...
gcloud batch jobs list --filter labels.source=\"inference_pipeline\" --sort-by ~createTime --format="value(name)" | awk -F '/' '{print $NF}' | xargs -I {} gcloud batch jobs delete "{}" --location="${GAR_LOCATION}" --quiet
