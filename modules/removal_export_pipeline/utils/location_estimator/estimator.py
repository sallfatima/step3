...
class Raycast:
    ...
    def get_raycast(self):
        return {
            "pitch": self.pitch + 0.5 * self.screen_y * self.fov / self.aspect,
    ...
    def get_distance(self):
        ...
        if -1 > theta > -89:
            d = abs(self.camera_height / math.tan(theta / 180.0 * math.pi))
            return d
        else:
            return None
    ...
    def intersection2d(self, segment):
        p2 = self.get_latlng()
        if self.camera_latlng == None or p2 == None:
            return None
        ...
        x4 = self.camera_latlng.get_xy(segment.end2).x
        y4 = self.camera_latlng.get_xy(segment.end2).y
        if (x4 * y2 - x3 * y2 - x2 * y4 + x2 * y3) == 0 or (
            y4 * x2 - y3 * x2 - y2 * x4 + y2 * x3
        ) == 0:
            return None
    ...
    def intersection(self, buidlings):
        ...
        inter_height = (
            mindis * math.tan(self.get_raycast()["pitch"] / 180.0 * math.pi)
            + self.camera_height
        )
