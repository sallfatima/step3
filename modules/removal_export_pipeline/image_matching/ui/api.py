...
from .utils import ROOT, filter_matches, get_feature_model, get_model, load_config
...
class ImageMatchingAPI(torch.nn.Module):
    ...
    def _updata_config(self):
        self.dense = self.conf["dense"]
        if not self.conf["dense"]:
    ...
    def get_nr_matches(
        ...
        """
        Get number of points left after applying ransac algorithm.
        """
        # plot images with ransac matches
        pred = self.pred
